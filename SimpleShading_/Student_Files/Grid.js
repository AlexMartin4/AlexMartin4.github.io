...
// centered at x=y=z=0.  Draw this shape using the GL_LINES primitive.
	...
	xcount = 30;			// # of lines to draw in x,y to make the grid.
	ycount = 30;		
	xymax	= 40.0;			// grid size; extends to cover +/-xymax in x and y.
	...
	for(v=0; v<2*ycount; v++) {
		if(v%2==0) {	// put even-numbered vertices at (xnow, -xymax, 0)
			appendPositions([-xymax, -xymax +v*ygap ,0.0,1.0]);								// w.
		}
		else {				// put odd-numbered vertices at (xnow, +xymax, 0).
			appendPositions([+xymax, -xymax +(v-1)*ygap, 0.0, 1.0]);										// w.
		}
...
function DrawGrid(Parent, index, mat1, mat2){
	
	updateModelMatrix(Parent);
	updateNormalMatrix(Parent);
	updateMaterial(mat1);
	gl.drawArrays(gl.LINES, index, 2*xcount);
	updateMaterial(mat2);
