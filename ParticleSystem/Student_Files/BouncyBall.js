...
PartSys.prototype.initBouncy = function(count) {
//==============================================================================
	...
	/*Walls = new CLimiter();
	Walls.initWalls(-2.0,2.0,-2.0,2.0, 0.0, 5.0, 0.95);
	this.limiters = this.limiters.concat(Walls.attributes);
	this.limitCount++;*/
	
	Cylinder = new CLimiter();
	Cylinder.initCylinder(2.0, 0.0, 5.0, 0.75);
	this.limiters = this.limiters.concat(Cylinder.attributes);
	this.limitCount++;
	
	Fountain = new CLimiter();
	Fountain.initFountain(this.lifeTime, 2.0, 0.0, 0.0, 3.0);
	this.limiters = this.limiters.concat(Fountain.attributes);
	this.limitCount++;
    ...
    for(var i = 0; i < this.partCount; i += 1, j+= PART_MAXVAR) {
      ...
      this.s1[j + PART_XVEL] =   0.0;//-15 + Math.random()*30;//this.INIT_VEL;
      this.s1[j + PART_YVEL] =   0.0;//-15 + Math.random()*30;//this.INIT_VEL;
      this.s1[j + PART_ZVEL] =   0.0;//-15 + Math.random()*30;
	  this.s1[j + PART_R] =  0.2;
      this.s1[j + PART_B] =  0.9;
      this.s1[j + PART_G] =  0.0;
      this.s1[j + PART_MASS] =  1.0;      // mass, in kg.
      this.s1[j + PART_DIAM] =  10+Math.random()*15;      // on-screen diameter, in pixels
  ...
  // Create a vertex buffer object (VBO) in the graphics hardware: get its ID# 
	this.shaderLoc = createProgram(gl, BOUNCY_VSHADER, BOUNCY_FSHADER); 
	if (!this.shaderLoc) {
