...
//==============================================================================	
	...
	Tornado = new CForcer();
	Tornado.initTornado(8, 30, 1, 5);
	this.forces = this.forces.concat(Tornado.attributes);
	this.forceCount++;
    ...
    for(var i = 0; i < this.partCount; i += 1, j+= PART_MAXVAR) {
      ...
      this.s0[j + PART_YPOS] =  2.0;      // with a 0.1 margin
      ...
      this.s1[j + PART_YPOS] =  2.0;      // with a 0.1 margin
      this.s1[j + PART_ZPOS] =  3.0;
      this.s1[j + PART_WPOS] =  1.0;      // position 'w' coordinate;

      this.s1[j + PART_XVEL] =  30.0*Math.random();//this.INIT_VEL;
      this.s1[j + PART_YVEL] =  30.0*Math.random();//this.INIT_VEL;
      this.s1[j + PART_ZVEL] =  30.0*Math.random();
	  this.s1[j + PART_R] =  0.0;
      this.s1[j + PART_B] =  0.2;
      this.s1[j + PART_G] =  0.8;
      this.s1[j + PART_MASS] =  1.0;      // mass, in kg.
      this.s1[j + PART_DIAM] =  5 + Math.random()*5;      // on-screen diameter, in pixels
  ...
  // Create a vertex buffer object (VBO) in the graphics hardware: get its ID# 
	this.shaderLoc = createProgram(gl, TORNADO_VSHADER, TORNADO_FSHADER); 
	if (!this.shaderLoc) {
